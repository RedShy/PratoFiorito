...
<body>
	...
	<div class="container">
		<div class="card card-login mx-auto text-center bg-dark">
			...
			<div class="card-body" >
							<h2>${lobby.host}</h2> ha creato la lobby 
			</div>
			<img style="height: 100px;" src="resources/images/angryMine.png" alt="Logo">
			      	...
			      	<c:if test="${ playerType eq 'host'}">
					      	...
					      	<form class="match" action="startGame">
					      		<div id="difficulty">
									...
									<input type="radio" name="difficulty" value="intermediate"> Medio
									<input type="radio" name="difficulty" value="advanced"> Difficile
								</div>
								<input id="startGameButton" class="btn btn-outline-danger  login_btn" type="submit" value="Start Game"  <c:if test="${empty lobby.guest}">disabled</c:if>   /> 
							...
							<c:if test="${ not empty noGuest}">
				      		...
				      		<c:if test="${ playerType eq 'host'}">
										...
										<c:if test="${not empty lobby.guest}">
											<div class="card-body" id="guestName">
												Gioco contro: ${lobby.guest}
											</div>	
										</c:if>		
			...
			<form action="exitLobby">
					<button type="submit" class="btn btn-default btn-lg  btn-danger">
					  <span class="glyphicon glyphicon-remove" aria-hidden="true"></span> Esci Dalla Lobby
					</button>
				</form>
		</div>
	</div>
